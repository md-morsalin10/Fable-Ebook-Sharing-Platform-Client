...
export default function AboutPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F17] text-gray-100 relative overflow-hidden select-none">
      ...
      <div className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-[#E5BA73]/5 rounded-full blur-[160px] pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-amber-500/[0.02] rounded-full blur-[140px] pointer-events-none" />

      {/* 🌟 1. Hero Section */}
      <div className="max-w-4xl mx-auto text-center pt-20 pb-16 px-4 relative z-10">
        <span className="text-xs uppercase tracking-[0.25em] text-[#E5BA73] font-semibold bg-[#E5BA73]/5 px-4 py-1.5 rounded-full border border-[#E5BA73]/10">
          Our Story
        </span>
        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-bold text-white mt-6 tracking-wide leading-tight">
          Where Stories Find Their <br className="hidden sm:inline" />
          <span className="text-[#E5BA73]">Private Sanctuary</span>
        </h1>
        <p className="text-base sm:text-lg text-gray-400 font-light mt-6 max-w-2xl mx-auto leading-relaxed">
      ...
      {/* 🏛️ 2. The Vision / Mission Section */}
      <div className="max-w-5xl mx-auto px-4 py-12 relative z-10">
        <div className="bg-[#111622]/40 border border-gray-800/60 backdrop-blur-md rounded-3xl p-8 sm:p-12 shadow-2xl grid grid-cols-1 md:grid-cols-5 gap-8 items-center">
          ...
          <div className="md:col-span-2 bg-[#0B0F17] border border-gray-800/80 rounded-2xl p-6 text-center shadow-inner">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {coreValues.map((value) => {
            ...
            return (
              <div 
                key={value.id} 
                className={`bg-[#111622]/30 border border-gray-800/50 rounded-2xl p-6 transition-all duration-300 transform hover:-translate-y-1 ${value.border}`}
              >
      ...
      <div className="max-w-4xl mx-auto text-center py-20 px-4 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8">
          <Link href="/register" className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[#E5BA73] text-[#0B0F17] px-6 py-3.5 rounded-xl text-sm font-bold hover:bg-[#d4a75e] hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 shadow-xl shadow-amber-950/10">
            <span>Get Started For Free</span>
            <ArrowRight className="w-4 h-4" />
          </Link>
          <Link href="/dashboard" className="w-full sm:w-auto text-sm text-gray-400 hover:text-white border border-gray-800 hover:border-gray-700 px-6 py-3.5 rounded-xl transition-all font-medium">
            Explore Dashboard
          </Link>
        </div>
      </div>
